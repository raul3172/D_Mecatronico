...
# ── Presets del gripper en escala [0.0 = cerrado, 1.0 = abierto] ──
# El ESP32 mapea este rango a [0°, 110°].
GRIP_ABIERTO = 1.00   # abierto total (110°)
GRIP_CERRADO = 0.00   # cerrado total (0°)
...
class CalibrationDialog(QDialog):
    STEPS = [
        ...
        ( 500, "🏠 Riel → inicio de carrera. Esperando...",    True ),
        (2000, "🦾 Verificando motores Hiwonder (30°)",         False),
        (2000, "🦾 Motores → posición home",                    False),
        (1500, "✋ Gripper: Cerrado (0°)",                      False),
        (1500, "✋ Gripper: Abierto (110°)",                    False),
    ...
    def __init__(self, node: TeachAndRepeatNode, parent=None):
        ...
        self.bar.setRange(0, len(self.STEPS) - 1); self.bar.setValue(0)
        self.bar.setTextVisible(False)
        self.bar.setStyleSheet(
            "QProgressBar{border-radius:4px; background:#bdc3c7;} "
        ...
        self._fb_timer.timeout.connect(
            lambda: self.lbl_fb.setText(f"ESP32: {self.node.last_feedback}")
            if self.node.last_feedback else None
        )
    ...
    def _advance(self):
        ...
        self.lbl.setText(msg); self.bar.setValue(self._step)
        self._execute_step(self._step)
        self._step += 1
        QTimer.singleShot(500 if wait_esp else delay, self._advance)
    ...
    def _execute_step(self, step):
        ...
        elif step == 5:                 # Gripper Cerrado (0.0)
            p[4] = GRIP_CERRADO
            self.node.send_single_point(p, 1.2)
            self._pos[4] = GRIP_CERRADO
        elif step == 6:                 # Gripper Abierto (1.0)
            p[4] = GRIP_ABIERTO
            self.node.send_single_point(p, 1.2)
            self._pos[4] = GRIP_ABIERTO
...
class TeachPendantGUI(QWidget):
    ...
    def _build_ui(self):
        ...
        self.lbl_tel.setStyleSheet(
            "background:#1e272e; color:#0be881; padding:8px; "
        ...
        # Presets del gripper simplificados a Abierto / Cerrado
        gb_grip = QGroupBox("Efector Final (Gripper)")
        lay_grip = QHBoxLayout(gb_grip)
        for text, color, val in [
            ("Abierto (110°)", "#e74c3c", GRIP_ABIERTO),
            ("Cerrado (0°)",   "#27ae60", GRIP_CERRADO),
        ]:
            btn = QPushButton(text)
            btn.setStyleSheet(f"background:{color}; color:white; height:35px; font-weight:bold;")
            btn.clicked.connect(lambda _, v=val: self._set_gripper_preset(v))
        ...
        self.combo.addItems(["Grabar Postura Actual", "Reproducir Secuencia",
                             "Reproducir en Bucle", "Vaciar Memoria"])
        ...
        btn_estop.setStyleSheet(
            "background:#c0392b; color:white; height:40px; font-weight:bold; border-radius:4px;"
        )
    ...
    def _set_ui_state(self, enabled: bool):
        ...
        self.combo.setEnabled(enabled); self.spin_loops.setEnabled(enabled)
        self.btn_exec.setEnabled(enabled)
        color = "#1e272e" if enabled else "#f1c40f"
        txt   = "#0be881" if enabled else "#2c3e50"
        self.lbl_tel.setStyleSheet(
            f"background:{color}; color:{txt}; padding:8px; "
            f"font-family:Consolas,monospace; font-size:13px; border-radius:4px;"
        )
    ...
    def _set_all_except_rail(self, enabled: bool):
        ...
        self.combo.setEnabled(enabled); self.btn_exec.setEnabled(enabled)
    ...
    def _execute_movement(self, idx: int):
        ...
        if idx == 0:
            ...
            self.node.send_single_point(list(self.target_positions), duration_sec=0.05)
            timeout_ms = max(800, int((dist_m / 0.02) * 1000))
            self._rail_moving = True
            self._set_all_except_rail(False)
            self.controls[0][0].setEnabled(False); self.controls[0][1].setEnabled(False)
            ...
            if self._rail_moving:
                self.controls[idx][1].setValue(
                    round(math.degrees(self.target_positions[idx]) / step) * step
                )
                return
            target_rad = math.radians(raw)
            self.target_positions[idx] = target_rad
            self.controls[idx][0].setEnabled(False); self.controls[idx][1].setEnabled(False)
    ...
    def _release_rail_lock(self):
        ...
        self.controls[0][0].setEnabled(True); self.controls[0][1].setEnabled(True)
    ...
    def _check_arrival(self):
        ...
        arrived   = [i for i in self.locked_joints
                     if abs(current[i] - self.target_positions[i]) <= tolerance]
        for i in arrived:
            self.locked_joints.remove(i)
            self.controls[i][0].setEnabled(True); self.controls[i][1].setEnabled(True)
        if not self.locked_joints:
            self.arrival_timer.stop()

    def _set_gripper_preset(self, val_normalizado: float):
        self.target_positions[4] = val_normalizado
    ...
    def _update_telemetry(self):
        ...
        moving    = ("PRECAUCIÓN: ROBOT EN MOVIMIENTO..."
                     if not self.left_panel.isEnabled() else "ESTADO DEL ROBOT:")
        texto  = f"{moving}\n"
        texto += f"Riel:{cur[0]*1000:+.0f}mm{rail_icon}  "
        for i, name in enumerate(['Cintura', 'Hombro', 'Codo'], 1):
            icon = "⏳" if i in self.locked_joints else " "
            texto += f"{name}:{math.degrees(cur[i]):+.0f}°{icon}  "
            
        # Reflejamos matemáticamente el rango [0.0, 1.0] a [0°, 110°] en la terminal
        grados_gripper = cur[4] * 110.0
        texto += f"\nGripper:{grados_gripper:.0f}°"
    ...
    def _estop(self):
        ...
        for i, (sl, sp) in enumerate(self.controls):
            ...
            sl.blockSignals(False); sp.blockSignals(False)
        QMessageBox.critical(self, "SISTEMA DETENIDO",
                             "Paro de Emergencia accionado.\nMotores detenidos.")
